...
const getMyChats = TryCatch(async (req, res, next) => {
  ...
  const transformedChats = await Promise.all(chats.map(async ({ _id, name, members, groupChat }) => {
    ...
    const latestMessage = await Message.findOne({ chat: _id })
      .sort({ createdAt: -1 })
      .select('createdAt');
...
const updateChatBackground = TryCatch(async (req, res, next) => {
  ...
  const systemMessage = {
    content: `${user.name} changed the chat background`,
    messageType: 'system',
    systemType: 'background_change',
    sender: user._id,
    chat: chatId
  };

  // Save both changes
  const [savedChat, savedMessage] = await Promise.all([
    chat.save(),
    Message.create(systemMessage)
  ]);
  ...
  // Only emit NEW_MESSAGE event - this will be handled specially by the frontend
  emitEvent(req, NEW_MESSAGE, chat.members, {
    chatId,
    message: messageForRealTime
  });
